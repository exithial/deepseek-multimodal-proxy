# Scripts de Gestión para DeepSeek Vision Proxy
...
cd ~/Proyectos/deepseek-vision-proxy
...
rm -rf ~/Proyectos/deepseek-vision-proxy
```
...
curl -X POST http://localhost:7777/v1/chat/completions \
  ...
  -d '{"model": "vision-dsk-chat", "messages": [{"role": "user", "content": "Hola"}]}'
```
...
**Solución:**
```bash
chmod +x setup-deepseek-proxy.sh check-proxy-status.sh uninstall-proxy.sh
```
...
Type=simple
User=exithial
WorkingDirectory=/home/exithial/Proyectos/deepseek-vision-proxy
...
ExecStart=/home/exithial/.nvm/versions/node/v24.13.0/bin/node /home/exithial/Proyectos/deepseek-vision-proxy/dist/index.js
Restart=on-failure
RestartSec=10
...
PrivateDevices=true
ProtectHome=true
ReadWritePaths=/home/exithial/Proyectos/deepseek-vision-proxy
...
1. Ejecuta `./check-proxy-status.sh` para diagnóstico
